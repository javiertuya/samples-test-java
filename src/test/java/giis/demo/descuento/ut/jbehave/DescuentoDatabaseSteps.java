...
public class DescuentoDatabaseSteps {
	// puesto que un mismo test ejecutara varios pasos se deben guardar valores generados en un paso para uso en el
	// siguiente
	private Database db;
	private int edad;
	...
	public void setClientes(ExamplesTable clientes) {
		...
		for (Map<String, String> row : clientes.getRows()) {
			db.executeUpdate(sql, row.get("id"), row.get("edad"), row.get("nuevo"), row.get("cupon"),
					row.get("tarjeta"));
		}
	}
